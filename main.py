...
client = pymongo.MongoClient("mongodb+srv://Jopt0598:10zXJAWWPZcRV3tH@rpscluster.zi6jn.mongodb.net/notes?retryWrites=true&w=majority")
...
def main_interface(user_information):
    while True:
        try:
            ...
            print_options(["1. Add a new note", "2. Delete a note", "3. Logout"])
            option = int(input("Your selection is: "))
            assert option == 1 or option == 2 or option == 3, "Select a valid option"
            if option == 1: add_note(user_information)
            if option == 2: delete_note(user_information, index)
            if option == 3: logout()
        except AssertionError as ve:
            print("")
            print(ve.args[0])
